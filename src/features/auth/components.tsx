...
export function OAuthButtons() {
  ...
  async function handleOAuth(provider: "google" | "github") {
    ...
    try {
      ...
      // If successful, the server action will trigger a redirect and not return.
    } catch (error: any) {
      // In Next.js, redirect() throws an error that should be rethrown
...
export function SubmitButton({
  ...
  return (
    <Button
      type="submit"
      disabled={loading}
      className={cn("w-full h-9 text-sm relative", className)}
    >
      {loading && (
